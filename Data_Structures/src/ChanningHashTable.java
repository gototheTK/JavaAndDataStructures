...
public class ChanningHashTable {
    ...
    public ChanningHashTable(int capacity) {
        if (capacity <= 0) {
            throw new RuntimeException("정상적인 해시테이블의 크기를 정해주세요.");
        }
    ...
    private void putValue(Object key, Object value) {
        /**
         * 조사된 버킷의 위치에 저장된 값이 null이 아니고 이미 동일한 키가 존재하면 값을 덮어쓴다.
    ...
    public void romove(Object key) {
        ...
        if (null != bucket[index]) {
            ...
            // TODO : 엔트리 리시트에 엔트리가 비어있으면 버킷을 비워준다.
            if (list.isEmpty()) {
                bucket[index] = null;
                --bucketSize;
            }
